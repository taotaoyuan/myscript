...
cat eggnog.emapper.annotations | sed '1,4d' | awk 'BEGIN{FS="\t";OFS="\t"}{print $6}' | awk NF | wc -l  # 5,508 predicted gene names
...
for species in $(cat species.list);
  do
    cat proteins-raw/$species.fa | seqkit seq -n | awk '{print $1}' | sed "s/^/$species-/g" > t1
